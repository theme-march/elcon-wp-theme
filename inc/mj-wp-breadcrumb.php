<?php
    ...
    if ( ! function_exists( 'mj_wp_breadcrumb' ) ) {
        function mj_wp_breadcrumb ( $list_style = 'ol', $list_id = 'breadcrumb', $list_class = 'breadcrumb  justify-content-center', $active_class = 'active', $echo = false ) {
            ...
            // Front page
            if ( is_front_page() ) {
                $breadcrumb .= '<li class="' . $active_class . ' breadcrumb-item">' . get_bloginfo( 'name' ). '</li>';
            } else {
                $breadcrumb .= '<li class="breadcrumb-item"><a href="' . home_url() . '">' . get_bloginfo( 'name' ) . '</a></li>';
            }
            ...
            if ( 'page' == get_option( 'show_on_front' ) && get_option( 'page_for_posts' ) ) {
                ...
                if ( is_home() ) {
                    $breadcrumb .= '<li class="' . $active_class . ' breadcrumb-item">' . get_the_title( $blog_page_id )  . '</li>';
            ...
            if ( is_archive() && ! is_tax() && ! is_post_type_archive() ) {
                $breadcrumb .= '<li class="' . $active_class . ' breadcrumb-item">';
            ...
            if ( is_singular( 'post' ) ) {
                ...
                // Post title
                $breadcrumb .= '<li class="' . $active_class . ' breadcrumb-item">' . get_the_title() . '</li>';
            }
            ...
            // Search
            if ( is_search() ) {
                $breadcrumb .= '<li class="' . $active_class . ' breadcrumb-item">' . __( 'Search', $text_domain ) . '</li>';
            }

            // 404
            if ( is_404() ) {
                $breadcrumb .= '<li class="' . $active_class . ' breadcrumb-item">' . __( '404', $text_domain ) . '</li>';
            }

            // Custom Post Type Archive
			if ( is_post_type_archive() ) {
				$breadcrumb .= '<li class="' . $active_class . ' breadcrumb-item">' . post_type_archive_title( '', false ) . '</li>';
			}
			...
			if ( is_tax() ) {
				...
				// Term title
				$breadcrumb .= '<li class="' . $active_class . ' breadcrumb-item">' . single_term_title( '', false ) . '</li>';
			}
